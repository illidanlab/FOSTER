...
class TinyImages(torch.utils.data.Dataset):
    ...
    def __init__(self, transform=None, exclude_cifar=True):
        #i-9 /localscratch2/yushuyan/data/tinyimage/tiny_images.bin
        #i-7 /media/Research/jyhong/data/OE_ood_dataset/tiny_images.bin'
        data_file = open('/localscratch2/yushuyan/data/tinyimage/tiny_images.bin', "rb")
        ...
        if exclude_cifar:
            self.cifar_idxs = []
            with open('./utilsood/80mn_cifar_idxs.txt', 'r') as idxs:
                for idx in idxs:
