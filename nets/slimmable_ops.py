...
class SlimmableConv2d(nn.Conv2d, SlimmableOpMixin):
    ...
    def _forward_with_partial_weight(self, x, slim_bias_idx, out_slim_bias_idx=None):
        out_idx0, out_idx1, in_idx0, in_idx1 = self._compute_slice_bound(self.in_channels, self.out_channels, slim_bias_idx, out_slim_bias_idx)
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        for name, param in self._parameters.items():
            if param is not None:
                ...
                if name == 'weight':
                    in_idx_bias = self.in_channels * self.slim_bias_idx if not self.non_slimmable_in else 0
                    param = param[out_idx_bias:(out_idx_bias+self.out_channels), in_idx_bias:(in_idx_bias+self.in_channels)]
                elif name == 'bias' and param is not None:
...
class SlimmableLinear(nn.Linear, SlimmableOpMixin):
    ...
    def _forward_with_partial_weight(self, x, slim_bias_idx, out_slim_bias_idx=None):
        out_idx0, out_idx1, in_idx0, in_idx1 = self._compute_slice_bound(self.in_features, self.out_features, slim_bias_idx, out_slim_bias_idx)
        weight = self.weight[out_idx0:out_idx1, in_idx0:in_idx1]
        bias = self.bias[out_idx0:out_idx1] if self.bias is not None else None
