...
from utils.data_utils import DomainNetDataset, DigitsDataset, Partitioner, \
    CifarDataset, CifarDataset100, ClassWisePartitioner, extract_labels, TinyImageNet, STLDataset, ImageNet12


def compose_transforms(trns: list, image_norm):
    if image_norm == '0.5':
...
def get_central_data(name: str, domains: list, percent=1., image_norm='none',
    ...
    if name.lower() == 'digits':
        ...
        if image_norm == 'default':
            image_norm = 'torch'
        for domain in domains:
            if domain not in STLDataset.all_domains:
                raise ValueError(f"Invalid domain: {domain}")
        trn_train = [transforms.RandomCrop(96),
                     transforms.RandomHorizontalFlip(),
                     transforms.ToTensor()]
        trn_test = [transforms.ToTensor()]

        train_sets = [STLDataset(domain, split='train',
                                   transform=compose_transforms(trn_train, image_norm))
                      for domain in domains]
        test_sets = [STLDataset(domain, split='test',
                                  transform=compose_transforms(trn_test, image_norm))
                     for domain in domains]
        ...
        if image_norm == 'default':
            # follow https://github.com/tjmoon0104/pytorch-tiny-imagenet/blob/master/ResNet18_64.ipynb no need to norm
            image_norm = 'none'
        for domain in domains:
            if domain not in TinyImageNet.all_domains:
                raise ValueError(f"Invalid domain: {domain}")
        trn_test = [transforms.ToTensor()]
        trn_train = [transforms.RandomRotation(20),
        transforms.RandomHorizontalFlip(0.5),
        transforms.ToTensor(),
        transforms.Normalize([0.4802, 0.4481, 0.3975], [0.2302, 0.2265, 0.2262]),]


        train_sets = [TinyImageNet(split='train', transform=compose_transforms(trn_train, image_norm))]
        test_sets = [TinyImageNet(split='test', transform=compose_transforms(trn_test, image_norm))]
    elif name.lower() == 'imagenet':
        if image_norm == 'default':
            # follow https://github.com/tjmoon0104/pytorch-tiny-imagenet/blob/master/ResNet18_64.ipynb no need to norm
            image_norm = 'none'
        for domain in domains:
            if domain not in ImageNet12.all_domains:
                f
        ...
        train_sets = [ImageNet12(split='train', transform=compose_transforms(trn_train, image_norm), num_classes=12)]
        test_sets = [ImageNet12(split='test', transform=compose_transforms(trn_test, image_norm), num_classes=12)]
    else:
...
def prepare_digits_data(args, domains=['MNIST', 'SVHN'], shuffle_eval=False, n_class_per_user=-1,
    ...
    train_loaders, val_loaders, test_loaders, clients = make_fed_data(
        ...
        min_n_sample_per_share=10 if n_class_per_user > 3 else 16, n_class_per_user=n_class_per_user,
...
def prepare_cifar100_data(args, domains=['cifar100'], shuffle_eval=False, n_class_per_user=-1,
    ...
    train_sets, test_sets = get_central_data('cifar100', domains)
    ...
    return train_loaders, val_loaders, test_loaders, clients
def prepare_stl_data(args, domains=['stl10'], shuffle_eval=False, n_class_per_user=-1,
                       n_user_per_domain=1, partition_seed=42, partition_mode='uni', val_ratio=0.2,
                       eq_domain_train_size=True, subset_with_logits=False,
                       consistent_test_class=False,
                       ):
    train_sets, test_sets = get_central_data('stl10', domains)
...
def prepare_imagenet_data(args, domains=('tin',), shuffle_eval=False, n_class_per_user=-1,
                          n_user_per_domain=1, partition_seed=42, partition_mode='uni', val_ratio=0.2,
                          eq_domain_train_size=True, subset_with_logits=False,
                          consistent_test_class=False,):
    train_sets, test_sets = get_central_data(args.data, domains)

    train_loaders, val_loaders, test_loaders, clients = make_fed_data(
        train_sets, test_sets, args.batch, domains, shuffle_eval=shuffle_eval,
        partition_seed=partition_seed, n_user_per_domain=n_user_per_domain, partition_mode=partition_mode,
        val_ratio=val_ratio, eq_domain_train_size=eq_domain_train_size, percent=args.percent,
        min_n_sample_per_share=2, subset_with_logits=subset_with_logits,
        n_class_per_user=n_class_per_user,
        test_batch_size=args.test_batch if hasattr(args, 'test_batch') else args.batch,
...
def prepare_ImageNet_data(args, domains=('imagenet12',), shuffle_eval=False, n_class_per_user=-1,
                          n_user_per_domain=1, partition_seed=42, partition_mode='uni', val_ratio=0.2,
                          eq_domain_train_size=True, subset_with_logits=False,
                          consistent_test_class=False,):
